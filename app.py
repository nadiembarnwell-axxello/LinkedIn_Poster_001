...
# 1. Page Configuration (Executed first safely)
st.set_page_config(
...
st.markdown("""
    <style>
    .block-container { padding-top: 1rem !important; padding-bottom: 0rem !important; padding-left: 3rem !important; padding-right: 3rem !important; max-width: 100% !important; }
    ...
    div.stButton > button { border-radius: 6px; height: 2.8em; font-weight: bold; background-color: #E31B23 !important; color: white !important; border: none !important; }
    div.stButton > button:hover { background-color: #B91C1C !important; }
    .stTabs [data-baseweb="tab"] { font-weight: 700 !important; color: #4B5563 !important; }
    .stTabs [data-baseweb="tab"][aria-selected="true"] { color: #E31B23 !important; border-bottom-color: #E31B23 !important; }
    </style>
""", unsafe_allow_html=True)

# 2. Safe Asset Checking Pipeline (Prevents Silent Blank Crashes)
logo_main_path = "Axxello Only 1x3.png"
logo_icon_path = "AXP Logo XX.jpg"

# Safely check for files and warn visually instead of breaking down silently
logo_main = None
if os.path.exists(logo_main_path):
    try:
        logo_main = Image.open(logo_main_path)
    except Exception as e:
        st.warning(f"⚠️ Could not open logo image: {e}")
...
# Typography Loader Engine (Schibsted Grotesk with fallback tracking)
def get_custom_font(style="regular", size=24):
    ...
    else: # Linux / Cloud Server standard fallbacks
        font_dir = "/usr/share/fonts/truetype/dejavu/"
        font_map = {"regular": "DejaVuSans.ttf", "bold": "DejaVuSans-Bold.ttf", "italic": "DejaVuSans-Oblique.ttf"}
    try: 
        return ImageFont.truetype(font_dir + font_map[style], size)
    except: 
...
with col_header_logo:
    if logo_main:
        ...
        st.markdown("<h2 style='color: #E31B23; margin: 0;'>AXXELLO</h2>", unsafe_allow_html=True)
...
if template_file and new_photo_file:
    try:
        ...
        with col_preview:
            st.error(f"❌ Core processing crashed: {e}")
else:
    with col_preview:
        st.subheader("🖥️ Production Canvas Preview")
